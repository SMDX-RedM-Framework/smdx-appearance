RegisterServerEvent("smdx-appearance:server:saveOutfit", function(clothes, price, outfitName)
    local src = source
...
RegisterNetEvent('smdx-clothes:server:saveUseOutfit', function(clothes)
    local src = source
    local Player = SMDXCore.Functions.GetPlayer(src)
    if not Player then return end
    if clothes ~= nil then
...
RegisterServerEvent('smdx-clothes:DeleteOutfit')
AddEventHandler('smdx-clothes:DeleteOutfit', function(name)
    local src = source
    local _name = name
    local Player = SMDXCore.Functions.GetPlayer(src)
...
lib.callback.register('smdx-clothes:LoadClothes', function(source)
...
lib.callback.register('smdx-clothes:server:getOutfits', function(source)
    local Player = SMDXCore.Functions.GetPlayer(source)
    local outfit = {}
