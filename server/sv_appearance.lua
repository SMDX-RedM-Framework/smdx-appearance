...
local function CheckVersion()
    PerformHttpRequest('https://raw.githubusercontent.com/SMDX-RedM-Framework/smdx-appearance/main/version.txt', function(err, text, headers)
...
RegisterServerEvent('smdx-appearance:SaveSkin')
AddEventHandler('smdx-appearance:SaveSkin', function(skin, clothes, oldplayer)
    ...
    if oldplayer then
        ...
        TriggerClientEvent('smdx-spawn:client:setupSpawnUI', source, encode, true)
    end
end)

RegisterServerEvent('smdx-appearance:SetPlayerBucket')
AddEventHandler('smdx-appearance:SetPlayerBucket', function(b, random)
    if random then
        local BucketID = SMDXCore.Shared.RandomInt(1000, 9999)
...
RegisterServerEvent('smdx-appearance:LoadSkin')
AddEventHandler('smdx-appearance:LoadSkin', function()
    local _source = source
    local User = SMDXCore.Functions.GetPlayer(source)
    ...
    if skins[1] then
        ...
        TriggerClientEvent("smdx-appearance:ApplySkin", _source, decodedSkin, decodedClothes)
    else
        TriggerClientEvent("smdx-appearance:OpenCreator", _source)
    end
end)


RegisterServerEvent("smdx-appearance:deleteSkin")
AddEventHandler("smdx-appearance:deleteSkin", function(license, Callback)
    local _source = source
    local id
...
RegisterServerEvent('smdx-appearance:updategender', function(gender)
    local Player = SMDXCore.Functions.GetPlayer(source)
    local citizenid = Player.PlayerData.citizenid
