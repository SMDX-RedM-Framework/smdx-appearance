...
Citizen.CreateThread(function()
    ...
    PromptSetControlAction(selectLeft, SMDX.Prompt.MalePrompt)
    ...
    PromptSetControlAction(selectRight, SMDX.Prompt.FemalePrompt)
    str = CreateVarString(10, 'LITERAL_STRING', str)
    ...
    PromptSetControlAction(selectEnter, SMDX.Prompt.ConfirmPrompt)
    str = CreateVarString(10, 'LITERAL_STRING', str)
...
Citizen.CreateThread(function()
    local str = SMDX.CameraPromptText
    CameraPrompt = PromptRegisterBegin()
    PromptSetControlAction(CameraPrompt, SMDX.Prompt.CameraUp)
    PromptSetControlAction(CameraPrompt, SMDX.Prompt.CameraDown)
    str = CreateVarString(10, 'LITERAL_STRING', str)
    ...
    str = SMDX.RotatePromptText
    RotatePrompt = PromptRegisterBegin()
    PromptSetControlAction(RotatePrompt, SMDX.Prompt.RotateLeft)
    PromptSetControlAction(RotatePrompt, SMDX.Prompt.RotateRight)
    str = CreateVarString(10, 'LITERAL_STRING', str)
    ...
    str = SMDX.ZoomPromptText
    ZoomPrompt = PromptRegisterBegin()
    PromptSetControlAction(ZoomPrompt, SMDX.Prompt.Zoom1)
    PromptSetControlAction(ZoomPrompt, SMDX.Prompt.Zoom2)
    str = CreateVarString(10, 'LITERAL_STRING', str)
...
function StartPrompts()
    ...
    while IsInCharCreation do
        ...
        local label = CreateVarString(10, 'LITERAL_STRING', SMDX.GroupPromptText)
        PromptSetActiveGroupThisFrame(RoomPrompts, label)
    
        if IsControlPressed(2, SMDX.Prompt.CameraUp) then
        ...
        if IsControlPressed(2, SMDX.Prompt.CameraDown) then
            local CamCoords = GetCamCoord(CharacterCreatorCamera)
        ...
        if IsControlPressed(2, SMDX.Prompt.RotateLeft) then
            local heading = GetEntityHeading(PlayerPedId())
            SetPedDesiredHeading(PlayerPedId(), heading - 40)
        end
    
        if IsControlPressed(2, SMDX.Prompt.RotateRight) then
            local heading = GetEntityHeading(PlayerPedId())
            SetPedDesiredHeading(PlayerPedId(), heading + 40)
        end
    
        if IsControlPressed(2, SMDX.Prompt.Zoom1) then
            SetCamFov(CharacterCreatorCamera, GetCamFov(CharacterCreatorCamera) - 1.5)
        end
    
        if IsControlPressed(2, SMDX.Prompt.Zoom2) then
            SetCamFov(CharacterCreatorCamera, GetCamFov(CharacterCreatorCamera) + 1.5)
        end
    end
end
...
function EndCharacterCreatorCam(anim, anim1)
    ...
    TriggerServerEvent("smdx-appearance:SaveSkin", CreatorCache, clothesHashes)
end
...
function FotoMugshots()
    ...
    local FirstName = SMDXCore.Functions.GetPlayerData().charinfo.firstname
    local LastName = SMDXCore.Functions.GetPlayerData().charinfo.lastname
